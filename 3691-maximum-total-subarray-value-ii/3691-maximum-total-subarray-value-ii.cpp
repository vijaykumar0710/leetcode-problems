...
segTree[i] = {max(segTree[2 * i + 1].first, segTree[2 * i + 2].first), 
              min(segTree[2 * i + 1].second, segTree[2 * i + 2].second)};
}
pair<int,int> query(int st,int end,int i,int l,int r){
    if(l>end || r<st) return {INT_MIN, INT_MAX};
    if(l>=st && r<=end) return segTree[i];
    int mid = l + (r - l) / 2;
    pair<int, int> left = query(st, end, 2 * i + 1, l, mid);
    pair<int, int> right = query(st, end, 2 * i + 2, mid + 1, r);
    return {max(left.first, right.first),min(left.second, right.second)};
}
long long f(vector<int>&nums,int i,int j){
    int n=nums.size();
   pair<int,int> p=query(i,j,0,0,n-1);
